...
export default function SerpButton() {
  ...
  useEffect(() => {
    ...
    // ✅ Stop processor on page load (only once)
  ...
  const handleScrape = useCallback(async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        //@ts-ignore
        toast.info('🔄 Processor will stop automatically', { duration: 3000 });
        await fetchStats();
      } else {
  ...
  }, [selectedNiche, selectedLocation, maxResults, loading, stats]);

  const getSelectedNiche = () => NICHES.find(n => n.value === selectedNiche);
  const niche = getSelectedNiche();

  const isButtonDisabled = loading || (stats?.remainingToday === 0);

  // ✅ Memoize dropdown toggles for better performance
  const toggleNicheDropdown = useCallback(() => {
    if (!loading) setIsNicheOpen(!isNicheOpen);
  }, [loading, isNicheOpen]);

  const toggleResultsDropdown = useCallback(() => {
    if (!loading) setIsResultsOpen(!isResultsOpen);
  }, [loading, isResultsOpen]);
  ...
  const selectResults = useCallback((value: number) => {
    setMaxResults(value);
            ...
            {isNicheOpen && !loading && (
              <div className="absolute z-10 w-full mt-1 bg-white border border-gray-200 rounded-lg shadow-lg max-h-48 overflow-auto">
                {NICHES.map((item) => (
                  <button
                    key={item.value}
                    onClick={() => selectNiche(item.value)}
